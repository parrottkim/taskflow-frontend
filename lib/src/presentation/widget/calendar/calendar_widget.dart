...
class Calendar extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveInitialDate =
        initialDate != null && isSelectableDate(initialDate!)
        ? initialDate
        : null;
    final effectiveInitialStartDate =
        initialStartDate != null && isSelectableDate(initialStartDate!)
        ? initialStartDate
        : null;
    final effectiveInitialEndDate =
        initialEndDate != null && isSelectableDate(initialEndDate!)
        ? initialEndDate
        : null;
    ...
    final focusedDay = useState<DateTime>(
      effectiveInitialDate ??
          effectiveInitialStartDate ??
          normalizedMinimumDate ??
          DateTime.now(),
    );
    // selectedDay: 단일 선택 모드에서 선택된 날짜
    final selectedDay = useState<DateTime?>(effectiveInitialDate);
    // rangeStart, rangeEnd: 범위 선택 모드에서 시작 및 종료 날짜
    final rangeStart = useState<DateTime?>(effectiveInitialStartDate);
    final rangeEnd = useState<DateTime?>(effectiveInitialEndDate);
      ...
      return Row(
        ...
        children: [
          ...
          InkWell(
            ...
            highlightColor: colorScheme.primary.faint,
            splashColor: colorScheme.primary.subtle,
            child: Padding(
              ...
              if (_mode == SelectionMode.range) {
                if (isRangeStart) {
                  ...
                  backgroundDecoration = BoxDecoration(
                    ...
                    color: colorScheme.primary.faint,
                  );
        ...
        itemBuilder: (context, pageIndex) {
          ...
          return GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final isSelectable = isSelectableDate(day);
