...
class _DropdownDialog extends StatelessWidget {
  ...
  const _DropdownDialog({required this.title, required this.child});
    ...
    return Dialog(
      child: ConstrainedBox(
        constraints: BoxConstraints(
          maxWidth: 420.0,
          maxHeight: math.min(MediaQuery.sizeOf(context).height * 0.75, 560.0),
        ),
        child: Column(
          ...
          children: [
            Padding(
              padding: const EdgeInsets.fromLTRB(20.0, 12.0, 8.0, 12.0),
              child: Row(
                children: [
                  Expanded(
...
class ElevatedDropdownButton<T> extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dropdownItems = <T?>[if (allowNull) null, ...items];
                ...
                child: Material(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(8.0),
                      border: Border.all(
                        color: Theme.of(context).colorScheme.outline.muted,
                      ),
          ...
          child: ElevatedButton.icon(
            onPressed: () async {
              ...
              await showDialog<void>(
                context: context,
                builder: (dialogContext) => _DropdownDialog(
                  title: label,
                  child: ListView(
                    shrinkWrap: true,
                    padding: const EdgeInsets.symmetric(vertical: 8.0),
            ...
            label: Row(
              ...
              children: [
                if (value != null) itemBuilder(value as T) else label,
                if (isExpanded) Spacer(),
                if (showClose && value != null && onClear != null)
                  Padding(
            ...
            onPressed: () async {
              ...
              await showDialog<void>(
                ...
                builder: (dialogContext) => StatefulBuilder(
                  builder: (context, setState) => _DropdownDialog(
                    title: Text(label ?? ''),
                    child: ListView(
                      shrinkWrap: true,
                      padding: const EdgeInsets.symmetric(vertical: 8.0),
                      children: [
                        for (final item in items)
                          Material(
                            color: selectedItems.value.contains(item)
                                ? colorScheme.primary.faint
                                : Colors.transparent,
                            child: InkWell(
                              onTap: () {
                                final selection = List<T>.from(
                                  selectedItems.value,
                                );
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  children: [
                                    IgnorePointer(
                                      child: AppToggleButton(
                                        value: selectedItems.value.contains(
                                          item,
                                        ),
                                        onChanged: (_) {},
                                      ),
                      ...
                      children: [
                        if (selectedPath.value.isNotEmpty)
                          ListTile(
                            ...
                            title: selectedPath.value.length == 1
                                ? Text(label ?? '')
                                : itemBuilder(
                                    selectedPath
                                        .value[selectedPath.value.length - 2],
                                  ),
                            onTap: () {
                              final path = List<T>.from(selectedPath.value)
                                ..removeLast();
                              ...
                              currentSelectionIndex.value = path.length - 1;
                              setState(() {});
                            },
                          ),
                        for (final item in currentLevelItems.value)
                          InkWell(
                            onTap: () {
                              final path = [...selectedPath.value, item];
                              final nextItems = getNextLevelItems(path, items);
                              selectedPath.value = path;
                              onChanged?.call(path);
                              currentLevelItems.value = nextItems;
                              currentSelectionIndex.value = path.length - 1;
